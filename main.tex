...
\[ \text{(MTTF) } \lambda_p^{-1} = 4.26 \]
\par The results indicate that this part has a mean time to failure (MTTF) of 4,255,319 hours. This makes sense, because the simplicity and fixed nature of the buck converter means it will last longer under load. The reliability could be increased by finding a more reliable part with better construction, or by using active cooling to decrease temperature fluctuations. Otherwise, this part is easily serviceable on the product, as it's physically separated from other, more sensitive ICs.
...
$$ \text{(MTTF) } \lambda_p^{-1} = 12.99 $$
\par The results indicate that this part has a mean time to failure (MTTF) of 12,990,000 hours.  This result is significantly higher than that of the buck converter from before, which makes intuitive sense. The LDO should run cooler and have fewer points of failure compared to a switching supply. To increase the reliability, better cooling or a Mil-Spec part could be considered.
\newpage
\paragraph{Inductor}
\begin{table}[htbp!]
\centering
...
$$ \text{(MTTF) } \lambda_p^{-1} = 15.000054 $$
\par The results indicate that this part has a mean time to failure (MTTF) of 15,000,054 hours.  This is due to the fixed nature of the inductor, as an alternative to a variable inductor which may wear more quickly. This also means it will fail after some other parts in the system, so reliability concerns are best focused on the previous parts.
...
\paragraph{Microcontroller}
\par The schematic for the microcontroller and related circuitry can be seen in Figure \ref{microcontroller}.
\subparagraph{Failure Conditions}
\par When reviewing the microcontroller, the most apparent source of failure is the many forms of decoupling used to balance the various power and ground inputs to the controller.
...
\paragraph{USB}
\par The schematic for the USB hub, ports, and related circuitry can be seen in Appendix A, Figure \ref{usb-a}.
\subparagraph{Failure Conditions}
...
\par The schematic for the USB-to-serial conversion circuitry can be seen in Appendix A, Figure \ref{usb-c}.
\subparagraph{Failure Conditions}
...
\par This section will explain constraints incurred by the development process given the hardware specifications and general system layouts. Then, given these constraints, the analysis will deduce a set of distinct software modules that can be developed, tested, and completed separately, while also maintaining their connection to the whole system.
...
\quickfigure{images/pcb-in-case.png}{15cm}{Harm Drone PCB Inside Case Housing}{pcb-in-case}

\quickfigure{images/pcb-in-case-front.png}{15cm}{Completely Sealing the PCB in the Case}{pcb-in-case-front}

\quickfigure{images/pcb-in-case-side.png}{15cm}{Side View of Harm Drone in Casing}{pcb-in-case-side}
...
This appendix outlines the Project-Specific Success Criteria (PSSC) as established during the proposal phase and verified during final testing. The criteria reflect the primary functional goals of the Harm Drone system and were used to assess the project's success. 

The original PSSCs were developed in collaboration with the project sponsor, Dr. Andrew Harms, and were later refined during system development. A change request was submitted to reflect updated project scope and design constraints; the signed approval is included below.

\subsubsection*{Original PSSCs (Proposal Phase)}
\begin{enumerate}
    \item The drone will autonomously navigate to 4 different user-defined GPS Coordinates, return, then land.
    \item The drone will maintain flight during wind speeds of up to 10 nautical miles per hour.
    \item The drone will use bi-directional USB communication to support up to 4 add-on sensors.
    \item The drone will contain a removable storage medium that will store up to 4GB of data.
    \item The drone’s lighting system will comply with IEEE 1789-2015 flicker standards.
\end{enumerate}

\subsubsection{Change Approval Letter}

\subsubsection*{Modified PSSCs (Final Version)}
\begin{enumerate}
    \item The board will receive and store 1000KB of flight, log, and sensor data on nonvolatile, persistent onboard memory. Testing involves loading high-entropy data and verifying recovery after power loss.
    \item The board will store content of sensor data on a removable SD card using a file system readable by Windows, macOS, and Linux.
    \item The board will support bi-directional USB communication with up to four sensors.
    \item The removable storage medium will store at least 4GB of data.
    \item The board’s lighting system will meet IEEE 1789-2015 Recommended Practice 2.
\end{enumerate}
...
\quickfigure{images/USB-timing.jpg}{15cm}{An example of USB packet timing.}{usb-timing}
\quickfigure{images/spi-timing.png}{15cm}{An example of SPI timing.}{spi-timing}
\clearpage
...
\begin{table}[htbp!]
	...
	\begin{tabular}{|lll|}
\hline
\multicolumn{3}{|c|}{Loading Analysis}                                                  \\ \hline
\multicolumn{3}{|c|}{15V Devices}                                                       \\ \hline
\multicolumn{1}{|l|}{Device No.}     & \multicolumn{1}{l|}{Device Descriptor} & Current \\ \hline
\multicolumn{1}{|l|}{LMR51430XDDCR}  & \multicolumn{1}{l|}{Buck Converter}    & 300 mA  \\ \hline
\multicolumn{3}{|c|}{5V Devices}                                                        \\ \hline
\multicolumn{1}{|l|}{AE11131-ND}     & \multicolumn{1}{l|}{USB 2.0 Ports}     & 100 mA  \\ \hline
\multicolumn{1}{|l|}{FT231XS-R}      & \multicolumn{1}{l|}{USB to UART}       & 8 mA    \\ \hline
\multicolumn{1}{|l|}{XC6210A332MR-G} & \multicolumn{1}{l|}{LDO}               & 100 mA  \\ \hline
\multicolumn{3}{|c|}{3.3V Devices}                                                      \\ \hline
\multicolumn{1}{|l|}{STM32F072CB}    & \multicolumn{1}{l|}{Microcontroller}   & 120 mA  \\ \hline
\multicolumn{1}{|l|}{CAT25160VI-GT3} & \multicolumn{1}{l|}{EEPROM}            & 2 mA    \\ \hline
\multicolumn{1}{|l|}{TUSB2046I}      & \multicolumn{1}{l|}{USB Hub}           & 40 mA   \\ \hline
\end{tabular}
\end{table}
\par This table lists the current draw for all of the components used to finish our PSSC's. The Buck converter receives 15 V from the battery, and steps it down to 5 V for the LDO, USB to UART, and the USB 2.0 ports. The LDO steps down to 3.3 V for the Microcontroller, EEPROM, and the USB Hub.
\clearpage
...
%\end{enumerate}
\quickfigure{software-level0}{12cm}{Top-level breakdown of the Harm Drone software stack.}{software-level0}
\quickfigure{software-level1}{16cm}{Low-level breakdown of the Harm Drone software stack.}{software-level1}
\quickfigure{software-routines}{16cm}{The main and flight routines of the integrated Harm Drone system.}{software-routines}
\quickfigure{software-integration-block-diagram}{17cm}{A typical software setup for a host drone utilizing the Harm Drone.}{software-integration-block-diagram}
\clearpage
...
\subfile{sections/appendix-f/carter.tex}
\subfile{sections/appendix-f/toby.tex}
\subfile{sections/appendix-f/camryn.tex}
\subfile{sections/appendix-f/mike.tex}
