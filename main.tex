...
\paragraph{Buck}\par The schematic for the buck converter and related circuitry can be seen in Appendix C, Figure \ref{buck}.
\subparagraph{Failure Conditions}
...
\paragraph{LDO}
\par The schematic for the LDO and related circuitry can be seen in Appendix C, Figure \ref{ldo}.
\subparagraph{Failure Conditions}
...
\paragraph{SPI}
\par The schematic for the SPI communication with the SD card, SRAM, and EEPROM can be seen in Appendix C, Figure \ref{spi}.
\subparagraph{Failure Conditions}
...
\paragraph{USB}
\par The schematic for the USB hub, ports, and related circuitry can be seen in Appendix C, Figure \ref{usba}.
\subparagraph{Failure Conditions}
...
\par The schematic for the USB-to-serial conversion circuitry can be seen in Appendix C, Figure \ref{usbc}.
\subparagraph{Failure Conditions}
...
\begin{enumerate}
    \item \textbf{Data Persistence:} The board will receive and store 1000KB of flight, log, and sensor data on nonvolatile, persistent onboard storage. This will be tested by writing 1000KB of high-entropy random data to the memory, then powering off the device for 60 seconds to allow for complete discharge. The test will pass if the data read from persistent storage matches the original data with an efficacy rate equal to that of the recovered data.

    \item \textbf{Cross-Platform SD Card Support:} The board will store content on a removable SD card using a filesystem accessible by all three major operating systems: Windows, macOS, and Debian Linux. We will verify this by writing 4GB of data to the SD card and successfully reading it from all three systems.

    \item \textbf{Sensor Communication Support:} The board will support bi-directional USB communication with up to four add-on sensors. This will be validated by connecting four USB sensors simultaneously and verifying real-time data transmission through each port.

    \item \textbf{Removable Data Logging:} The board will utilize a removable storage medium to store up to 4GB of data. Testing will involve writing 4GB of data to the medium and reading the full contents from an external system to confirm data integrity.

    \item \textbf{Flicker-Free Lighting Compliance:} The board’s lighting system will comply with IEEE 1789-2015 Recommended Practice 2, section 8.1.1, which defines flicker modulation limits as follows:
    \begin{itemize}
        \item Below 90 Hz: Modulation (\%) < $0.01 \times$ frequency
        \item Between 90–3000 Hz: Modulation (\%) < $0.0333 \times$ frequency
        \item Above 3000 Hz: No restriction on modulation
    \end{itemize}
    This criterion will be validated using an oscilloscope to measure LED modulation frequency and ensure compliance with biological safety thresholds.
\end{enumerate}

\vspace{1cm}

\noindent\textbf{SPO Signature:} Herb Detloff
